...
const Log_in = (props) => {
    ...
    const addtoDB = async ()=>{      //로그인 정보에 대한 데이터 테이블 생성
        try{
            ...
            props.navigation.navigate("test1") // 데이터 셋을 만듦과 동시에 다음 화면으로 넘어가도록 추가
        } catch(error){
    ...
    return (
        <View>
            <Text>QUIZ APP FOR STUDENT</Text>
            ...
            <Button title = "LogIn" onPress={addtoDB} style = {styles.buttonView}
                // onPress = {()=>{
                // {addtoDB}
